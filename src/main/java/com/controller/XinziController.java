    ...
    */
    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(xinzi !=null){
            ...
            //级联表 老师
            //级联表
            LaoshiEntity laoshi = laoshiService.selectById(xinzi.getLaoshiId());
            if(laoshi != null){
    ...
    */
    ...
    public R save(@RequestBody XinziEntity xinzi, HttpServletRequest request){
        ...
        if(false)
            ...
            xinzi.setLaoshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<XinziEntity> queryWrapper = new EntityWrapper<XinziEntity>()
            .eq("laoshi_id", xinzi.getLaoshiId())
        ...
        if(xinziEntity==null){
            ...
            xinzi.setShifaJine(xinzi.getJixiaoJine()+xinzi.getJiangjinJine()+xinzi.getButieJine()+xinzi.getJibenJine()-xinzi.getKouchuJine());//薪资表特有,不需要可以手动删掉
            xinziService.insert(xinzi);
            return R.ok();
        }else {
            return R.error(511,"该老师该月已有薪资记录");
        }
    }
...
//                            xinziEntity.setLaoshiId(Integer.valueOf(data.get(0)));   //老师 要改的
...
//                            xinziEntity.setYingdaoNumber(Integer.valueOf(data.get(0)));   //应到天数 要改的
//                            xinziEntity.setShidaoNumber(Integer.valueOf(data.get(0)));   //实到天数 要改的
//                            xinziEntity.setKouchuJine(data.get(0));                    //扣除金额 要改的

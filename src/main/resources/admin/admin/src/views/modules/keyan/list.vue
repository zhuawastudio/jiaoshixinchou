        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '科研名称' : ''">
                         <el-input prefix-icon="el-icon-search" v-model="searchForm.keyanName" placeholder="科研名称" clearable></el-input>
                     </el-form-item>
                 
                     <el-form-item :label="contents.inputTitle == 1 ? '科研类型' : ''">
                        <el-select v-model="searchForm.keyanTypes" placeholder="请选择科研类型">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in keyanTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('keyan','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jiaoshixinchou/upload/keyanMuBan.xls"
                        >批量导入科研数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('keyan','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="keyanUploadSuccess"
                                :on-error="keyanUploadError"
                        ...
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('keyan','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "keyan.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="keyanUuidNumber"
                                   header-align="center"
                                   label="科研编号">
                        <template slot-scope="scope">
                            {{scope.row.keyanUuidNumber}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      ...
                                      label="科研类型">
                        <template slot-scope="scope">
                            {{scope.row.keyanValue}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="keyanKaishiTime"
                                   header-align="center"
                                   label="科研开始时间">
                        <template slot-scope="scope">
                            {{scope.row.keyanKaishiTime}}
                        </template>
                    </el-table-column>

                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                   prop="keyanJieshuTime"
                                   header-align="center"
                                   label="科研结束时间">
                        <template slot-scope="scope">
                            {{scope.row.keyanJieshuTime}}
                        </template>
                    </el-table-column>
                    <el-table-column :sortable="contents.tableSortable" :align="contents.tableAlign" prop="keyanChengguoFile"
                                     header-align="center"
                                     width="200"
                                     label="成果附件">
                        <template slot-scope="scope">
                            <div v-if="scope.row.keyanChengguoFile">
                                <a type="text" style="text-decoration:none" class="el-button" :href="$base.url+scope.row.keyanChengguoFile">下载</a>
                            </div>
                            <div v-else>无文件</div>
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('keyan','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('keyan','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('keyan','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              keyanTypesSelectSearch : [],
            form:{
                id : null,
                laoshiId : null,
                keyanUuidNumber : null,
                keyanName : null,
                keyanAddress : null,
                keyanTypes : null,
                keyanContent : null,
                keyanKaishiTime : null,
                keyanJieshuTime : null,
                keyanChengguoFile : null,
                keyanChengguoContent : null,
                insertTime : null,
                createTime : null,
            },
            dataList: [],
            ...
            json_fields: {
                //级联表字段
                     ...
                     '账户状态': 'jinyongTypes',
                //本表字段
                     '科研编号': "keyanUuidNumber",
                     '科研名称': "keyanName",
                     '科研地点': "keyanAddress",
                     '科研类型': "keyanValue",
                     '科研开始时间': "keyanKaishiTime",
                     '科研结束时间': "keyanJieshuTime",
                     '成果附件': "keyanChengguoFile",
                     '录入时间': "insertTime",
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'keyan',//当前表表名,
                        sumColum : 'keyan_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'keyan_name',//分组字符串字段
                        // types : 'keyan_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
            ...
            getDataList() {
                ...
                if (this.searchForm.keyanName!= '' && this.searchForm.keyanName!= undefined) {
                    params['keyanName'] = '%' + this.searchForm.keyanName + '%'
                }
                 
                if (this.searchForm.keyanTypes!= '' && this.searchForm.keyanTypes!= undefined) {
                    params['keyanTypes'] = this.searchForm.keyanTypes
                }
                                                        
                params['keyanDelete'] = 1// 逻辑删除字段 1 未删除 2 删除
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=keyan_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.keyanTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            keyanUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "keyan/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({

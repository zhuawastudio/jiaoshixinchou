        ...
        <div v-if="showFlag">
            <el-form :inline="true" :model="searchForm" class="form-content">
                <el-row :gutter="20" class="slt" :style="{justifyContent:contents.searchBoxPosition=='1'?'flex-start':contents.searchBoxPosition=='2'?'center':'flex-end'}">
                     ...
                     <el-form-item :label="contents.inputTitle == 1 ? '考勤结果' : ''">
                        <el-select v-model="searchForm.laoshikaoqinTypes" placeholder="请选择考勤结果">
                            <el-option label="=-请选择-=" value=""></el-option>
                            <el-option
                               v-for="(item,index) in laoshikaoqinTypesSelectSearch"
                               v-bind:key="index"
                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           v-if="isAuth('laoshikaoqin','导入导出')"
                           icon="el-icon-download"
                           href="http://localhost:8080/jiaoshixinchou/upload/laoshikaoqinMuBan.xls"
                        >批量导入老师考勤数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('laoshikaoqin','导入导出')"
                                ...
                                :action="getActionUrl"
                                :on-success="laoshikaoqinUploadSuccess"
                                :on-error="laoshikaoqinUploadError"
                                :show-file-list = false>
                            <el-button
                                    v-if="isAuth('laoshikaoqin','导入导出')"
                                    type="success"
                                    icon="el-icon-upload2"
                            >批量导入老师考勤数据</el-button>
                        </el-upload>
                        &nbsp;
                        <!-- 导出excel -->
                        <download-excel v-if="isAuth('laoshikaoqin','导入导出')" style="display: inline-block" class = "export-excel-wrapper" :data = "dataList" :fields = "json_fields" name = "laoshikaoqin.xls">
                            <!-- 导出excel -->
                            <el-button
                                    type="success"
                                    icon="el-icon-download"
            ...
            <div class="table-content">
                <el-table class="tables" :size="contents.tableSize" :show-header="contents.tableShowHeader"
                    ...
                    <el-table-column  :sortable="contents.tableSortable" :align="contents.tableAlign"
                                      prop="laoshikaoqinTypes"
                                      header-align="center"
                                      label="考勤结果">
                        <template slot-scope="scope">
                            {{scope.row.laoshikaoqinValue}}
                        </template>
                    </el-table-column>
                    ...
                    <el-table-column width="300" :align="contents.tableAlign"
                        ...
                        <template slot-scope="scope">
                            ...
                            <el-button v-if="isAuth('laoshikaoqin','查看')" type="success" icon="el-icon-tickets" size="mini" @click="addOrUpdateHandler(scope.row.id,'info')">详情</el-button>
                            <el-button v-if="isAuth('laoshikaoqin','修改')" type="primary" icon="el-icon-edit" size="mini" @click="addOrUpdateHandler(scope.row.id)">修改</el-button>
                            <el-button v-if="isAuth('laoshikaoqin','删除')" type="danger" icon="el-icon-delete" size="mini" @click="deleteHandler(scope.row.id)">删除</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
              laoshikaoqinTypesSelectSearch : [],
            form:{
                id : null,
                laoshiId : null,
                laoshikaoqinTypes : null,
                laoshikaoqinContent : null,
                kaoqinTime : null,
                insertTime : null,
                createTime : null,
            },
        ...
        methods: {
            chartDialog() {
                ...
                let params = {
                    ...
                    thisTable : {//当前表
                        tableName :'laoshikaoqin',//当前表表名,
                        sumColum : 'laoshikaoqin_number', //求和字段
                        date : 'insert_time',//分组日期字段
                        // string : 'laoshikaoqin_name',//分组字符串字段
                        // types : 'laoshikaoqin_types',//分组下拉框字段
                    },
                    // joinTable : {//级联表（可以不存在）
                ...
                _this.$nextTick(() => {
                    ...
                    this.$http({
                        url: "barCountTwo",//barCountOne barCountTwo barSumOne barSumTwo
                        method: "get",
                        params: {
                          riqi :getMonthFormat(_this.echartsDate),//年月
                        }
                ...
                //         groupColumn: "laoshikaoqin_types",
                //// riqi :getMonthFormat(_this.echartsDate),//年月
                //     }
                //     this.$http({
            ...
            getDataList() {
                ...
                if (this.searchForm.laoshikaoqinTypes!= '' && this.searchForm.laoshikaoqinTypes!= undefined) {
                    params['laoshikaoqinTypes'] = this.searchForm.laoshikaoqinTypes
                }
                                
                params['laoshikaoqinDelete'] = 1// 逻辑删除字段 1 未删除 2 删除


                this.$http({
                    url: "laoshikaoqin/page",
                    method: "get",
                    params: params
                }).then(({data}) => {
                    if(data && data.code === 0){
                ...
                //填充下拉框选项
                this.$http({
                    url: "dictionary/page?dicCode=laoshikaoqin_types&page=1&limit=100",
                    method: "get",
                }).then(({data}) => {
                    if(data && data.code === 0){
                        this.laoshikaoqinTypesSelectSearch = data.data.list;
                    }
                });
            },
            //每页数
            ...
            // 导入功能上传文件成功后调用导入方法
            laoshikaoqinUploadSuccess(data){
                let _this = this;
                _this.$http({
                    url: "laoshikaoqin/batchInsert?fileName=" + data.file,
                    method: "get"
                }).then(({data}) => {
                    if(data && data.code === 0){
                        _this.$message({
